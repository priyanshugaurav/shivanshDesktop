...
import { 
    ...
    RefreshCw, MinusCircle, PackagePlus
} from 'lucide-react';
...
const SpareStockDashboard = ({ theme: t }) => {
    ...
    const handleRestock = async () => {
        if (!restockItem || !restockQty || restockQty <= 0) return alert("Enter a valid quantity.");
        try {
            await axios.patch(`${API_URL}/spare-stocks/${restockItem._id}/restock`, { qty: Number(restockQty) }, getAuthHeader());
            setRestockItem(null);
            setRestockQty('');
            fetchStocks(selectedCategory._id);
            fetchCategories();
    ...
    // Manual Stock Out: Mark item as out of stock
    const handleManualStockOut = async (item) => {
        if (!window.confirm(`Mark "${item.name}" as OUT OF STOCK? This will set quantity to 0.`)) return;
        try {
            await axios.patch(`${API_URL}/spare-stocks/${item._id}/manual-stock-out`, {}, getAuthHeader());
            fetchStocks(selectedCategory._id);
            fetchCategories();
        } catch (err) {
            console.error(err);
            alert(err.response?.data?.error || "Failed to mark as out of stock");
        }
    };
                    ...
                    <div className="flex-1 p-8 bg-slate-50/50 overflow-y-auto">
                        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Manual Stock Out Checkbox */}
                                <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                                    <input 
                                        type="checkbox"
                                        id="manual-stock-out"
                                        checked={stockForm.manualStockOut}
                                        onChange={(e) => setStockForm({...stockForm, manualStockOut: e.target.checked})}
                                        className="w-5 h-5 rounded border-amber-300 text-amber-600 focus:ring-amber-500 cursor-pointer"
                                    />
                                    <label htmlFor="manual-stock-out" className="cursor-pointer">
                                        <span className="text-xs font-bold text-amber-800 block">Manual Stock Out Item</span>
                                        <span className="text-[10px] text-amber-600 font-medium">Enable for tangible items (e.g. Grease, Oil) sold in small amounts. You can manually mark it as out of stock anytime.</span>
                                    </label>
                                </div>
                                            ...
                                            <tr key={item._id} className="border-b border-slate-50 transition-colors group hover:bg-slate-50/80">
                                                ...
                                                <td className="py-3 px-4">
                                                    <div className="flex items-center gap-2">
                                                        <span className="font-bold text-sm text-slate-700">{item.name}</span>
                                                        {item.manualStockOut && (
                                                            <span className="text-[8px] font-black uppercase tracking-wider bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">Manual</span>
                                                        )}
                                                    </div>
                                                </td>
                                                <td className="py-3 px-4 text-right">
                                                ...
                                                <td className="py-3 px-4 text-right">
                                                    <div className="flex justify-end gap-1">
                                                        ...
                                                        {/* Manual Stock Out Button */}
                                                        {item.manualStockOut && item.qty > 0 && (
                                                            <button 
                                                                onClick={() => handleManualStockOut(item)}
                                                        ...
                                                        <button 
                                                            onClick={() => {
                                                                ...
                                                                setStockForm({
                                                                    ...
                                                                    status: item.status,
                                                                    manualStockOut: item.manualStockOut || false
                                                                });
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-5">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-emerald-50 rounded-lg text-emerald-600"><PackagePlus size={20}/></div>
                                <div>
                                    <h3 className="text-lg font-black text-slate-800">Restock Item</h3>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase">{restockItem.name}</p>
                                </div>
                            </div>
                            <button onClick={() => setRestockItem(null)} className="text-slate-400 hover:text-slate-600"><X size={20}/></button>
                        </div>
                        <div className="bg-slate-50 rounded-xl p-4 mb-5">
                            <div className="flex justify-between items-center text-sm">
                                <span className="font-bold text-slate-500">Current Stock</span>
                                <span className={`font-black ${restockItem.qty > 0 ? 'text-emerald-600' : 'text-red-500'}`}>{restockItem.qty} units</span>
                            </div>
                        </div>
                        <div className="mb-6">
                            <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5">Add Quantity</label>
                            <input 
                                autoFocus
                                type="number" 
                                min="1"
                                ...
                                className="w-full h-12 px-4 rounded-xl border border-slate-200 font-bold text-slate-800 text-lg focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none bg-white"
                                placeholder="Enter quantity to add..."
                            />
                            {restockQty > 0 && (
                                <p className="text-xs font-bold text-emerald-600 mt-2">New stock will be: {(restockItem.qty || 0) + Number(restockQty)} units</p>
                            )}
                        </div>
                        <div className="flex gap-3">
                            <button 
                            ...
                            <button 
                                ...
                                className="flex-1 py-3 bg-emerald-600 text-white rounded-xl font-bold uppercase text-[10px] hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/30 disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2"
                            >
